...
Authentication is configured via the `auth` field using `KafkaAuth` from `faucet-kafka-common`. The full auth reference is in the [`faucet-kafka-common` README](../kafka-common/README.md#auth-modes).
...
- [`faucet-kafka-common`](../kafka-common/README.md) — shared auth modes, value formats, schema registry client, and policy enums.
