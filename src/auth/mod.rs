...
impl Auth {
    pub fn apply(&self, headers: &mut HeaderMap) -> Result<(), FaucetError> {
        match self {
            Auth::None => Ok(()),
            Auth::Bearer(token) => { bearer::apply(headers, token); Ok(()) }
            Auth::Basic { username, password } => { basic::apply(headers, username, password); Ok(()) }
            Auth::ApiKey { header, value } => api_key::apply(headers, header, value),
            Auth::OAuth2 { .. } => Ok(()),
            Auth::Custom(h) => { custom::apply(headers, h); Ok(()) }
        }
    }
}
