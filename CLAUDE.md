...
Cargo workspace, 47 crates (46 libraries + the `faucet-cli` binary). All connector crates depend only on `faucet-core`; the umbrella `faucet-stream` and the `faucet-cli` binary depend on every connector + state crate via optional features.

| Crate | Path | Description |
|-------|------|-------------|
| `faucet-core` | `crates/core/` | Shared types, traits (`Source`, `Sink`, `AuthProvider`), pipeline orchestration, transforms, error types |
| `faucet-auth` | `crates/auth/` | Shared single-flight auth providers (`StaticProvider`, OAuth2 client-credentials / refresh-with-rotation, token-endpoint) implementing `faucet_core::AuthProvider` |
...
- `stage.rs` — `TransformStage`: the pipeline-level stage type wrapping the unchanged `RecordTransform` (1→1) as `Map`, plus `Filter` (1→0|1) and `Explode` (1→0..N). Includes a hand-rolled v1 JSONPath subset parser (`CompiledPath`) for bare keys / dot paths / bracketed string keys; explode mutates parent containers, so we don't use `jsonpath_rust` here. `apply_stages` is the per-record runner; `instrumented_apply_stages` (in `observability/transform.rs`) wraps it with per-page counters.
- `transform.rs` — `RecordTransform` / `CompiledTransform`: built-ins — `flatten`, `rename_keys` (regex), `keys_case` (with `KeyCaseMode` — snake/camel/pascal/kebab/screaming_snake), `select`, `drop`, `set`, `rename_field`, `cast` (with `CastType` + `CastOnError`), `redact`, `value_case` (with `ValueCaseMode`), `spell_symbols` (with a built-in symbol → word map + optional `extra` overrides) — plus `Custom` closures. Built-in transforms are feature-gated (`transform-<name>`); the `transforms` aggregate enables all of them. The legacy `KeysToSnakeCase` / `transform-snake-case` was removed in favour of `KeysCase` — it was never relied on by external configs. `filter` and `explode` are stage-level transforms, not `RecordTransform` variants — see `stage.rs`.
- `transforming_source.rs` — `TransformingSource`: wraps any `Source` with a fixed `Vec<TransformStage>`, applied per page via `instrumented_apply_stages`. The canonical way library callers attach transforms to any source; the CLI uses this same type internally (no private wrapper).
...
Top-level YAML/JSON shape: `version: 1`, optional `name:`, required `pipeline: { source, sink, transforms, state }`, optional `matrix: [...]` (per-row overrides + parent/child fan-out), optional `execution: { max_concurrent, on_error }`, optional `auth: { <name>: { type, config } }` (shared auth providers — see below). Each matrix row is deep-merged into `pipeline`; rows with `parent: <id>` run once per record produced by the parent, with `${parent_id.dotted.path}` tokens resolved per-record at runtime. Empty matrix = one anonymous invocation. State key is `{name}::{row_id}` for roots and `{name}::{row_id}::{parent_record_key}` for children. The full grammar lives in `cli/README.md`. Named source/sink templates under `pipeline.sources` / `pipeline.sinks` + a top-level `vars:` block are the recommended shape for multi-row configs; the legacy singular `pipeline.source` / `pipeline.sink` continues to work and registers as the `default` template.

**Auth model.** Every connector's `auth`/`credentials` config serializes as one consistent shape: `{ type: <method>, config: { … } }` (adjacent-tagged, snake_case discriminators) — e.g. `auth: { type: bearer, config: { token: … } }`. The connector's `auth` field is `faucet_core::AuthSpec<XxxAuth>`: it accepts **either** that inline form **or** `auth: { ref: <name> }`, which points at a provider in the top-level **`auth:` catalog** (`{ <name>: { type, config } }`, types: `static` / `oauth2` / `oauth2_refresh` / `token_endpoint`, built by `faucet-auth`). The CLI builds each named provider **once** (`cli/src/auth_catalog.rs`) and injects the same `Arc<dyn AuthProvider>` into every connector that references it (`registry::build_source`/`build_sink` call `with_auth_provider`), so N matrix rows hitting one IdP share a single token with single-flight refresh instead of racing. Library callers do the same via `Source::with_auth_provider(Arc<dyn AuthProvider>)`. Eight connectors consume shared providers (rest, graphql, xml, grpc, websocket, sink-http, elasticsearch, snowflake); Kafka/BigQuery/GCS keep the consistent wire shape but are not bearer/token-based so they don't take `auth: { ref }`. **This `{type,config}` shape was a breaking change** (issue #113) — pre-1.0; all examples/READMEs were migrated.

**Transforms layering.** Transforms can be declared at three layers and are resolved additively per matrix row in lifecycle order: `T_pipeline ++ T_source ++ T_row`. Each declaring layer (source template, matrix row) carries an `inherit_transforms: bool` (default `true`); setting it `false` drops every layer above. Sinks reject both `transforms:` and `inherit_transforms:` with typed errors — destination shaping belongs at the pipeline or row level. Library callers wrap any source with `faucet_core::TransformingSource` (no per-connector escape hatch — the previous `RestStreamConfig::add_transform` was deleted). See `docs/book/src/cookbook/transforms.md` for the full model.
...
- `config.rs` — `PipelineConfig { version, name, vars: Option<HashMap<String, Value>>, auth: Option<HashMap<String, Value>>, pipeline: PipelineSpec, matrix: Vec<MatrixRow>, execution: Option<ExecutionSpec> }` and `OnError { Continue, Stop }`. The `auth:` block is the shared-provider catalog.
- `auth_catalog.rs` — `build_auth_catalog` turns the `auth:` block into `HashMap<String, SharedAuthProvider>` (built once, shared) via `faucet_auth::build_provider`; `auth_ref` extracts a connector's `auth: { ref }`; `resolve` looks it up (typed `UnknownAuthProvider`). `PipelineSpec.source` / `sink` are `Option<ConnectorSpec>` (legacy singular shape); named templates live under `pipeline.sources` / `pipeline.sinks: HashMap<String, ConnectorSpec>`. `PartialConnector` gains `r#ref: Option<String>` for template selection per matrix row. `MatrixRow` carries `Option<PartialConnector>` overrides for source/sink. Rejects `version != 1` and the pre-#54 top-level `source:`/`sink:` shape (with a clear hint).
...
- `transforms.rs` — `compile_transforms`: every transform exposed via config (`flatten`, `rename_keys`, `keys_case`, `select`, `drop`, `set`, `rename_field`, `cast`, `redact`, `value_case`, `spell_symbols`); custom-closure transforms remain Rust-only. Per-transform decode structs live in this module (each derives `JsonSchema`); the support enums (`CastType`, `CastOnError`, `KeyCaseMode`, `ValueCaseMode`) are re-exported from `faucet_core` and also derive `JsonSchema`. `transform_descriptions()` provides one-line summaries for `faucet list`, and `transform_schema(name)` returns the JSON Schema for `faucet schema transform <name>`.
- `commands/` — `run` calls `expand` + `run_expanded`; `validate` calls `expand` and reports one line per row; `preview` runs the first root only (children need parent records to resolve `${parent.path}`); `list` renders sources, sinks, and transforms (each as a two-column block) plus the state-store names; `schema` covers `source` / `sink` / `transform` / `dlq`. `init` is schema-driven: it accepts `--source` / `--sink` (and an optional positional `name`), pulls each kind's JSON Schema via `registry::source_schema` / `sink_schema`, and renders a comment-annotated YAML through `init_template::schema_to_yaml_template` — required fields are surfaced with `# REQUIRED` and a typed placeholder; optional fields are commented out so connector-level defaults stand. The `--interactive` mode is gated behind the `cli-interactive` Cargo feature and uses `inquire` for the TTY Select prompt.
...
- `faucet_transform_records_in_total{pipeline,row}` (renamed from `_records_total` in 2026-05-29), `faucet_transform_records_out_total`, `faucet_transform_duration_seconds`, `faucet_transform_errors_total{...,kind}`
...
Shared auth: `auth` pulls in `faucet-auth` (OAuth2 / token-endpoint providers) so library users can build shared providers; the CLI always links `faucet-auth` for the top-level `auth:` catalog.

Aggregate features: `source` (all sources), `sink` (all sinks), `state` (all state backends), `auth` (shared auth providers), `full` (everything). Kafka-only: `kafka-schema-registry` enables Avro / Protobuf / JSON Schema via Confluent Schema Registry.
