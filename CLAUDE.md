...
- `transforming_source.rs` — `TransformingSource`: wraps any `Source` with a fixed `Vec<RecordTransform>`, applied per page via `instrumented_apply_all`. The canonical way library callers attach transforms to any source; the CLI uses this same type internally (no private wrapper).
