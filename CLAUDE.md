...
- `executor.rs` — `run_expanded(nodes, ExecuteOptions)`. One bounded `Semaphore` shared across roots and per-parent-record child fan-outs. `on_error: stop` runs sequentially within a level for deterministic halt-on-first-failure semantics; `on_error: continue` runs in parallel via `JoinSet` and skips a failed node's subtree. State-key collisions among siblings under a single parent surface as `CliError::DuplicateStateKey`. Wraps each invocation's source with a `StateKeyOverride` so per-row state keys override the source's natural one.
