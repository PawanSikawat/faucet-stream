...
/// function (`staging:` block, #528). The load SQL/URL generators are
/// unit-tested; the server-side execution is not exercised in CI (no live
/// ClickHouse + object store), same as Redshift/Snowflake. MSSQL `COPY INTO`
/// remains the follow-up tail of #528.
pub const STAGED_LOAD_SINK_KINDS: &[&str] = &["redshift", "snowflake", "bigquery", "clickhouse"];
...
mod tests {
    ...
    fn staged_load_allowlist_matches_capable_sinks() {
        for k in ["redshift", "snowflake", "bigquery", "clickhouse"] {
            assert!(
                sink_supports_staged_load(k),
                "{k} should support staged load"
            );
        }
        // mssql `COPY INTO` is the remaining follow-up tail of #528.
        for k in ["jsonl", "mssql", "postgres", "stdout"] {
            assert!(
                !sink_supports_staged_load(k),
